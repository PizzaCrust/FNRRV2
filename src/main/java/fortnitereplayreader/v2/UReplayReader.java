...
public abstract class UReplayReader {
    ...
    protected byte[] decompress(byte[] bytes) throws Exception {
        if (!this.replay.meta.isCompressed) {
            return bytes;
        }
        BinaryReader decrypted = new BinaryReader(new ByteArrayInputStream(bytes));
        int decompressedSize = decrypted.readInt32();
        int compressedSize = decrypted.readInt32();
        byte[] payload = decrypted.readBytes(compressedSize);
        return Oodle.INSTANCE.decompress(payload, decompressedSize);
    }
    ...
    private void parseReplayData() throws Exception {
        ...
        //TODO
        byte[] decrypted = decrypt((int) length);
